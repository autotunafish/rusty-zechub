...
rawTx=$txJSON
block=$(cat $rawTx | jq .height)

transparent=$(cat $rawTx | jq .vout[].scriptPubKey.type)
transparent=("${transparent[@]}")

transparent2=$(cat $rawTx | jq .vin[].txid | wc -l )

sprout=$(cat $rawTx | jq .vjoinsplit)
sapling1=$(cat $rawTx | jq .vShieldedSpend)
sapling2=$(cat $rawTx | jq .vShieldedOutput)
orchard1=$(cat $rawTx | jq -r '.orchard.actions | select( . != null )')



# Get Value Out
valueOut1=$(cat $rawTx | jq .valueBalance)
valueOut2=$(cat $rawTx | jq .orchard.valueBalance)
valueOut3=$(cat $rawTx | jq .vout[].valueZat | paste -sd+ | bc)



isCoinbase=$(cat $rawTx | jq .vin[] | grep coinbase)
...
if [ "$isDebug" == "true" ]; then
    ...
    fee=$(./getTXfee.sh $txJSON)
fi

# Get transferTX count
t=$(cat $rawTx $txID | jq -r '.vout | length' | paste -sd+ | bc)
o=$(cat $rawTx $txID | jq .orchard | jq -r '.actions | length' | paste -sd+ | bc)
s=$(cat $rawTx $txID | jq -r ' .vShieldedOutput | length ' | paste -sd+ | bc)
...
##		testTime="In mempool"
##	else
##                now=$(cat $rawTx | jq .height | xargs -n1 ./toCurl.sh getblock | jq .time)
##                testTime=$(date -d @$now +%c)
##	fi
##else
#zcashd case
now=$(cat $rawTx | jq .time)
testTime=$(date -d @$now +%c)
##fi

txID=$(cat $rawTx | jq .txid)
