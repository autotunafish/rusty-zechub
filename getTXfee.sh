...
txJSON="${1}"    #1 represent 1st argument
isShieldedOut=0
isShieldedIn=0
...
#Check for Orchard
isOrchard=$(cat $rawTx | jq .orchard)
lenO=${#isOrchard}



#Check for Sapling
isSaplingSpend=$(cat $rawTx | jq .vShieldedSpend)
isSaplingOutput=$(cat $rawTx | jq .vShieldedOutput)
isSapling=0
...
#Check for Sprout
isSprout=$(cat $rawTx | jq .vjoinsplit)
lenS=${#isSprout}
...
#Check vouts
nullCheck=$(cat $rawTx | jq .vout[])

if [[ -n "$nullCheck" ]] && [[ "$nullCheck" != "" ]];then
    
    voutSum=$(cat $rawTx | jq -r '.vout[] | .valueZat' | awk '{s+=$1} END {OFMT="%f";print s}')
    #If coinbase, add lockbox portion see line 63
    #check for mix txids here
    shieldCheck=$(cat $rawTx | jq -r '.orchard.actions | length')
    nullCheck=$(cat $rawTx | jq .vin[])
...
if [[ -n "$nullCheck" ]] && [[ "$nullCheck" != "" ]];then
    ...
    isCoinbase=$(cat $rawTx | jq .vin[] | grep -o coinbase)
    if [[ "$isCoinbase" = "coinbase" ]];then
        ...
        if [[ "$nullCheck" == "[]" ]];then
           ...
           #loop through vins
           length=$(cat $rawTx | jq -r '.vin | length')
           index=0
           if [ -f temp.md ]; then
    		rm temp.md
	   fi
	   ...
	   while [[ "$index" -lt "$length" ]]
		   do
		   	tempIndex=$(cat $rawTx | jq .vin[$index].vout)
		   	vinSum=$(cat $rawTx | jq .vin[$index].txid | xargs -n1 ./txDetails.sh | jq .vout[$tempIndex].valueZat)
		       	echo $vinSum >> temp.md
		        index=$((($index + 1)))
...
if [[ "$isShieldedOut" -eq 1 ]];then
    if [[ "$lenS" -gt 4 ]];then
       outValueBalance=$(cat $rawTx | jq .vjoinsplit[].vpub_newZat | awk '{s+=$1} END {print s}')
    elif [[ "$isSapling" -eq 1 ]];then
        outValueBalance=$(cat $rawTx | jq .valueBalanceZat)
        if [[ -n "$isOrchard" ]] && [[ "$lenO" -gt 4 ]];then
                myOrchard=$(cat $rawTx | jq .orchard.valueBalanceZat)
        	outValueBalance=$(echo "$outValueBalance + $myOrchard" | bc)
	fi
    elif [[ -n "$isOrchard" ]] && [[ "$lenO" -gt 4 ]];then
        outValueBalance=$(cat $rawTx | jq .orchard.valueBalanceZat)    
    else
        echo "debug"
    fi
...
if [[ "$isShieldedIn" -eq 1 ]];then
   #if [ "$lenS" -gt 2 ];then
   #   vinSum=$(cat $rawTx | jq .vjoinsplit[].vpub_newZat)
   if [[ "$isSapling" -eq 1 ]];then
       inValueBalance=$(cat $rawTx | jq .valueBalanceZat)
        if [[ -n "$isOrchard" ]] && [[ "$lenO" -gt 4 ]];then
              myOrchard=$(cat $rawTx | jq .orchard.valueBalanceZat)
              inValueBalance=$(echo "$inValueBalance + $myOrchard" | bc)
	fi
   elif [[ -n "$isOrchard" ]] && [[ "$lenO" -gt 4 ]];then
       inValueBalance=$(cat $rawTx | jq .orchard.valueBalanceZat)    
   else
       echo "debug"
   fi
fi
